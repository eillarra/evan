...
class Registration(models.Model):
    ...
    def pending_amount(self) -> str:
        ...
        saldo = ("%.2f" % (-self.saldo / 100.00)) if self.event.currency in ['EUR', 'USD'] else self.saldo
        return '{0}{1}'.format(symbols[self.event.currency], saldo)
...
def registration_post_save(sender, instance, created, *args, **kwargs):
    if created:
        msg = EmailMessage(
            ...
            from_email='{0} <{1}>'.format(instance.event.hashtag.upper(), settings.SYMPO_REGISTRATIONS_EMAIL)
        )
        msg.send()
