...
from evan.services.pdf import Pdf, PdfResponse
...
class CertificatePdfMaker:
    ...
    def make_pdf(self):
        ...
        intro = f"""
On behalf of the {event.name} organizing committee, I would hereby like to confirm that **{reg.user.name}**
— {reg.user.affiliation} — attended {event}
...
"""
        ...
        with Pdf() as pdf:
            ...
            if reg.letter.submitted:
                paper = f"""
{reg.user.name} has submitted a {reg.letter.submitted} titled \u2018{reg.letter.submitted_title}\u2019
...
{reg.user.name} registered for {event}
