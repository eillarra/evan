var Store = new Vuex.Store({
    ...
    getters: {
        ...
        topics: function (state) {
            ...
            return state.event.topics.sort(textSort('name'));
        },
        tracks: function (state) {
            if (!state.event || state.event.tracks == []) return [];
            return state.event.tracks.sort(textSort('name'));
        },
    ...
    mutations: {
        ...
        getEvent: function (state) {
            ajax().get($('#vue').data('url-event')).then(function (res) {
                state.event = res;
            });
        },
...
var EventView = {
    ...
    computed: _.extend(
        ...
        markedPresentation: function () {
            if (!this.event.presentation) return '';
            return marked(this.event.presentation, {sanitize: true});
        }
    }),
    methods: {
...
var VenuesView = {
    ...
    computed: _.extend(Vuex.mapState(['saving', 'event']), {
        items: function () {
            ...
            return this.event.venues.sort(textSort('name'));
        },
        selectedId: function () {
