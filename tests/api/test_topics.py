...
from evan.utils.factories import EventFactory, TopicFactory, UserFactory
...
class TestForAnonymous:
    ...
    def test_update(self, api_client, topic) -> None:
        data = self._get_update_data()
        response = api_client.put(topic.get_api_url(), data)
        assert response.status_code == self.expected_status_codes["update"]

    def test_delete(self, api_client, topic) -> None:
        response = api_client.delete(topic.get_api_url())
...
class TestForEventManager(TestForAuthenticated):
    ...
    def test_update_for_other_event(self, api_client) -> None:
        other_event = EventFactory()
        other_topic = TopicFactory(event=other_event)
        response = api_client.put(other_topic.get_api_url(), self._get_update_data())
        assert response.status_code == status.FORBIDDEN
