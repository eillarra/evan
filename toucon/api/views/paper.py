...
class PapersViewSet(ConferenceRelatedCreateOnlyViewSet):
    ...
    def upload(self, request, *args, **kwargs):
        ...
        for record in sheet.to_records():
            bulk_papers.append(Paper(**{**dict(record), **{'conference_id': conference.id}}))

        Paper.objects.bulk_create(bulk_papers)
        return Response(PaperSerializer(conference.papers.all(), many=True, context={'request': request}).data)
